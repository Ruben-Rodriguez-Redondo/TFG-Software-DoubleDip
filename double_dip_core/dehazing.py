...
class Dehaze(object):
    ...
    def _init_images(self):
        ...
        # Todo check, dind't use augmentations, always take the firts (the original)
    ...
    def _obtain_current_result(self, iter):
        ...
        if iter % 8 == 0:
            ...
            psnr = compare_psnr(self.images[0].astype(np.float32),
    ...
    def _plot_closure(self, iter):
        ...
        sys.stdout.write(f'\rIteration {iter:05d}    '
                         f'Loss {self.total_loss.item():.6f} '
                         f' current_psnr:{self.current_result.psnr:.6f} '
                         f' max_psnr {self.best_result.psnr:.6f} ')
        sys.stdout.flush()
        # Save images
        # Todo change this and maybe some of the other to save in iterations and in the finalize the correspondant things
...
def main_dehaze(img_path, conf_params={}):
    """
    ...
    dh = Dehaze(image_name + "_{}".format(1), dh.post, **conf_params)
    dh.optimize()
    dh.finalize()
